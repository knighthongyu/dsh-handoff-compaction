...
export class HandoffCompactionEngine extends BasicCompactionEngine {
    ...
    async summarize(input, agent, signal) {
        ...
        if (sourceMessageCount === 0) {
            ...
            this.ctx.logger.warn(`handoff validation failed: code=${error.code} sourceMessages=0 provider=${target.provider} model=${target.model} inputTokens=unknown outputTokens=unknown cacheReadTokens=unknown`);
            throw error;
        }
        ...
        if (fullMessages.length < sourceMessageCount
            ...
            this.ctx.logger.warn(`handoff validation failed: code=${error.code} sourceMessages=${sourceMessageCount} surfaceMessages=${fullMessages.length} provider=${target.provider} model=${target.model} inputTokens=unknown outputTokens=unknown cacheReadTokens=unknown`);
            throw error;
        }
        ...
        for (let attempt = 0; attempt < MAX_HANDOFF_ATTEMPTS; attempt += 1) {
            ...
            try {
                ...
                this.ctx.logger.warn(`handoff validation failed: code=${error.code} sourceMessages=${sourceMessageCount} provider=${options.provider} model=${options.model} inputTokens=${usage?.inputTokens ?? 'unknown'} outputTokens=${usage?.outputTokens ?? 'unknown'} cacheReadTokens=${usage?.cacheReadTokens ?? 'unknown'} attempt=${attempt + 1}/${MAX_HANDOFF_ATTEMPTS}`);
                if (attempt === 0) {
                    recovery = true;
                    continue;
                }
